...
def main():
    ...
    parser.add_argument('-f',
                        '--flavor',
                        dest='flavor',
                        help="Specify an optional flavor, which will be used as a suffix on firmware signal names and start and end markers on the modules that have use_flavor=true attribute")

    parser.add_argument('-a',
                        '--address_table',
                        dest='address_table',
                        help="Optionally specify the address table XML file to be used -- this filename will override the filename in the config file, note that prefixes and suffixes will not be used on this filename")

    parser.add_argument('-u',
                        '--update_generated_file',
                        # nargs=0,
                        dest='update_generated',
                        help="If you specify 'true' here, then generated files will be used as source - this can be used to rerun the script e.g. with different flavor to update the generated files that were produced by the last run")
    ...
    global PREFIX
    global FLAVOR
    global GENERATED_FILE_AS_SOURCE
    ...
    if args.update_generated is not None and args.update_generated.lower() == "true":
        GENERATED_FILE_AS_SOURCE = True
    else:
        GENERATED_FILE_AS_SOURCE = False
    ...
    if args.address_table is not None:
        ADDRESS_TABLE_TOP = args.address_table
    ...
    write_constants_file(modules, CONSTANTS_FILE, PREFIX, SUFFIX)
...
def write_constants_file(modules, filename, prefix, suffix):

    def write_constants(filename, args):
        f = filename
        ...
        for module in modules:
            ...
            f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + '_NUM_REGS' + flav + ' : integer := ' + str(total_regs32) + ';\n')
            f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + '_ADDRESS_MSB' + flav + ' : integer := ' + str(module.reg_address_msb) + ';\n')
            f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + '_ADDRESS_LSB' + flav + ' : integer := ' + str(module.reg_address_lsb) + ';\n')

            for reg in module.regs:

                if (VERBOSE):
                    print('Writing register constants for ' + reg.name)
                f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + '_ADDR' + flav + '    : '\
                ...
                if reg.msb == reg.lsb:
                    f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + '_BIT' + flav + '    : '\
                                'integer := ' + str(reg.msb) + ';\n')
                else:
                    f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + '_MSB' + flav + '    : '\
                                'integer := ' + str(reg.msb) + ';\n')
                    f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + '_LSB' + flav + '     : '\
                                'integer := ' + str(reg.lsb) + ';\n')
                if reg.default==-1:
                    ...
                    f.write('    constant ' + VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + '_DEFAULT' + flav + ' : '\
    ...
    out_fname = updateFilename(filename, prefix, suffix)
    source_fname = filename if not GENERATED_FILE_AS_SOURCE else out_fname
...
def update_module_file(module, prefix, suffix, use_tmr):
    ...
    flav = "_" + FLAVOR if module.use_flavor and FLAVOR != "" else ""
    ...
    def write_signals (filename):
        signal_declaration        = "    signal regs_read_arr<flavor>        : t_std32_array(<num_regs> - 1 downto 0) := (others => (others => '0'));\n"\
            "    signal regs_write_arr<flavor>       : t_std32_array(<num_regs> - 1 downto 0) := (others => (others => '0'));\n"\
            "    signal regs_addresses<flavor>       : t_std32_array(<num_regs> - 1 downto 0) := (others => (others => '0'));\n"\
            "    signal regs_defaults<flavor>        : t_std32_array(<num_regs> - 1 downto 0) := (others => (others => '0'));\n"\
            "    signal regs_read_pulse_arr<flavor>  : std_logic_vector(<num_regs> - 1 downto 0) := (others => '0');\n"\
            "    signal regs_write_pulse_arr<flavor> : std_logic_vector(<num_regs> - 1 downto 0) := (others => '0');\n"\
            "    signal regs_read_ready_arr<flavor>  : std_logic_vector(<num_regs> - 1 downto 0) := (others => '1');\n" \
            "    signal regs_write_done_arr<flavor>  : std_logic_vector(<num_regs> - 1 downto 0) := (others => '1');\n" \
            "    signal regs_writable_arr<flavor>    : std_logic_vector(<num_regs> - 1 downto 0) := (others => '0');\n"
        signal_declaration = signal_declaration.replace('<num_regs>', VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + '_NUM_REGS' + flav.upper())
        signal_declaration = signal_declaration.replace('<flavor>', flav)
    ...
    def write_slaves (filename, args):
        ...
        slave_declaration = '    ipbus_slave_inst : entity work.%s\n' % slave_entity + \
                           ...
                           ('           g_ENABLE_TMR           => %s,\n' % ('EN_TMR_IPB_SLAVE_'     + module.get_vhdl_name()) if use_tmr else "") + \
                            '           g_NUM_REGS             => %s,\n' % (VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + '_NUM_REGS' + flav.upper()) + \
                            '           g_ADDR_HIGH_BIT        => %s,\n' % (VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + '_ADDRESS_MSB' + flav.upper()) + \
                            '           g_ADDR_LOW_BIT         => %s,\n' % (VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + '_ADDRESS_LSB' + flav.upper()) + \
                            ...
                            '           usr_clk_i              => %s,\n' % (module.user_clock) + \
                            tmr_output + \
                            '           regs_read_arr_i        => regs_read_arr%s,\n' % (flav) + \
                            '           regs_write_arr_o       => regs_write_arr%s,\n' % (flav) + \
                            '           read_pulse_arr_o       => regs_read_pulse_arr%s,\n' % (flav) + \
                            '           write_pulse_arr_o      => regs_write_pulse_arr%s,\n' % (flav) + \
                            '           regs_read_ready_arr_i  => regs_read_ready_arr%s,\n' % (flav) + \
                            '           regs_write_done_arr_i  => regs_write_done_arr%s,\n' % (flav) + \
                            '           individual_addrs_arr_i => regs_addresses%s,\n' % (flav) + \
                            '           regs_defaults_arr_i    => regs_defaults%s,\n' % (flav) + \
                            '           writable_regs_i        => regs_writable_arr%s\n' % (flav) + \
                            '      );\n'
        ...
        for i in range(0, total_regs32):
            ...
            bit_high = VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + "_ADDRESS_MSB" + flav.upper()
            bit_lo = VHDL_REG_CONSTANT_PREFIX + module.get_vhdl_name() + "_ADDRESS_LSB" + flav.upper()
            vhdl_adr = vhdl_hex_padded(unique_addresses[i], module.reg_address_msb - module.reg_address_lsb + 1)
            f.write('    regs_addresses%s(%d)(%s downto %s) <= %s;\n' % (flav, i, bit_high, bit_lo, vhdl_adr))
        f.write('\n')
        ...
        for reg in module.regs:
            ...
            if 'r' in reg.permission:
                ...
                if is_single_bit:
                    bitrange = VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + "_BIT" + flav.upper()
                else:
                    bitrange = VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + "_MSB" + flav.upper() + \
                        " downto " +VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + "_LSB" + flav.upper()
                f.write('    regs_read_arr%s(%d)(%s) <= %s;\n' % (flav, adr, bitrange, reg.signal))
        f.write('\n')
        ...
        for reg in module.regs:
            ...
            if 'w' in reg.permission and reg.signal is not None:
                ...
                if is_single_bit:
                    ...
                    bitrange = VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + "_MSB" + flav.upper() + \
                        " downto " + VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + "_LSB" + flav.upper()
                f.write('    %s <= regs_write_arr%s(%d)(%s);\n' % (reg.signal, flav, adr, bitrange))
        f.write('\n')
        ...
        for reg in module.regs:
            if 'w' in reg.permission and reg.write_pulse_signal is not None:
                ...
                f.write('    %s <= regs_write_pulse_arr%s(%d);\n' % (reg.write_pulse_signal, flav,
        ...
        for reg in module.regs:
            if 'w' in reg.permission and reg.write_done_signal is not None:
                ...
                f.write('    regs_write_done_arr%s(%d) <= %s;\n' % (flav, unique_addresses.index(reg.address),
        ...
        for reg in module.regs:
            if 'r' in reg.permission and reg.read_pulse_signal is not None:
                ...
                f.write('    %s <= regs_read_pulse_arr%s(%d);\n' % (reg.read_pulse_signal, flav,
                                                                  unique_addresses.index(reg.address)))
        f.write('\n')
        ...
        for reg in module.regs:
            if 'r' in reg.permission and reg.read_ready_signal is not None:
                ...
                f.write('    regs_read_ready_arr%s(%d) <= %s;\n' % \
                        (flav, unique_addresses.index(reg.address), reg.read_ready_signal))
        ...
        for reg in module.regs:
            ...
            if reg.default is not None:
                ...
                if is_single_bit:
                    bit_suffix = '_BIT' + flav.upper()
                else:
                    bit_suffix = '_MSB' + flav.upper() + ' downto ' + VHDL_REG_CONSTANT_PREFIX \
                        + reg.get_vhdl_name() + '_LSB' + flav.upper()

                if isinstance(reg.default, int):
                    f.write('    regs_defaults%s(%d)(%s) <= %s;\n' % \
                            (flav, unique_addresses.index(reg.address), \
                                VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + bit_suffix,
                                VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + '_DEFAULT' + flav.upper()))
                else:
                    f.write('    regs_defaults%s(%d)(%s) <= %s;\n' % \
                            (flav, unique_addresses.index(reg.address), \
                                VHDL_REG_CONSTANT_PREFIX + reg.get_vhdl_name() + bit_suffix,
        ...
        for reg_addr in writable_reg_addresses:
                f.write("    regs_writable_arr%s(%d) <= '1';\n" % (flav, reg_addr))
    ...
    source_fname = module.file_name if not GENERATED_FILE_AS_SOURCE else out_fname

    sig_start_marker = VHDL_REG_SIGNAL_MARKER_START + " " + FLAVOR if module.use_flavor and FLAVOR != "" else VHDL_REG_SIGNAL_MARKER_START
    sig_end_marker = VHDL_REG_SIGNAL_MARKER_END + " " + FLAVOR if module.use_flavor and FLAVOR != "" else VHDL_REG_SIGNAL_MARKER_END
    reg_start_marker = VHDL_REG_SLAVE_MARKER_START + " " + FLAVOR if module.use_flavor and FLAVOR != "" else VHDL_REG_SLAVE_MARKER_START
    reg_end_marker = VHDL_REG_SLAVE_MARKER_END + " " + FLAVOR if module.use_flavor and FLAVOR != "" else VHDL_REG_SLAVE_MARKER_END

    insert_code (source_fname, out_fname, sig_start_marker, \
                 sig_end_marker, write_signals)
    insert_code (out_fname, out_fname, reg_start_marker, \
                 reg_end_marker, write_slaves, {"has_ipb_clk_period_generic": has_ipb_clk_period_generic})
...
def process_module (name, node, modules, variables):
    ...
    if node.get('fw_is_module_external') is not None \
       and node.get('fw_is_module_external') == 'true':
        ...
        module.use_flavor = False
        if node.get('fw_use_flavor') is not None and node.get('fw_use_flavor').lower() == 'true':
            module.use_flavor = True
